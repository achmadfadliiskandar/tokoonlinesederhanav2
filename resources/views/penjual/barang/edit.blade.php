...
<div class="container">
<form action="{{url('penjualupdatebarang',$barang->id)}}" method="POST" enctype="multipart/form-data">
    @csrf
    @method('put')
  <div class="mb-3">
  ...
  <div class="mb-3">
    ...
    <select name="kategories_id" id="kategories_id" class="form-select form-control">
    <!-- <option value="" selected>Tidak Dipilih</option> -->
    @foreach($kategoris as $kategori)
    <option value="{{$kategori->id}}" @selected($kategori->id == $barang->kategories_id)>{{$kategori->kategori}}</option>
    @endforeach
    </select>
  </div>
  <div class="mb-3">
...
<a href="{{url('penjualbarang')}}" class="btn btn-warning my-3">Back</a>
</div>
@endsection
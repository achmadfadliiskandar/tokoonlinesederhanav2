...
@endif
<form action="{{url('adminstorebarang')}}" method="POST" enctype="multipart/form-data">
    @csrf
    <div class="mb-3">
    <label for="penjual_id" class="form-label">Nama Penjual</label>
    <select name="penjual_id" id="penjual_id" class="form-select">
        @foreach($penjual as $penjual)
        <option value="{{$penjual->id}}">{{$penjual->user->name}} -> {{$penjual->namatoko}}</option>
        @endforeach
    </select>
  </div>
  <div class="mb-3">

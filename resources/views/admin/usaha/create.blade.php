...
@endif
<form action="{{url('adminstoretoko')}}" method="post">
@csrf
<div class="mb-3">
  <label for="nama" class="form-label">Nama Toko</label>
  <input type="text" class="form-control @error('namatoko') is-invalid @enderror" id="nama" name="namatoko" value="{{old('namatoko')}}">
</div>
<div class="mb-3">
    ...
    <option value="{{$user->id}}" @selected($user->id == $user->id)>{{$user->name}} - {{$user->role}}</option>
    @endforeach
    </select>
</div>
<div class="mb-3">
  <label for="alamat" class="form-label">Alamat Toko</label>
  <textarea class="form-control @error('alamattoko') is-invalid @enderror" id="alamat" name="alamattoko" rows="3">{{old('alamattoko')}}</textarea>
</div>
<div class="mb-3">
  <label for="kodepos" class="form-label">Kode Pos</label>
  <input type="number" class="form-control @error('kodepos') is-invalid @enderror" id="kodepos" name="kodepos" value="{{old('kodepos')}}">
</div>
<div class="mb-3">
  <label for="cabangtoko" class="form-label">cabang Toko</label>
  <input type="number" class="form-control @error('cabangtoko') is-invalid @enderror" id="cabangtoko" name="cabangtoko" value="{{old('cabangtoko')}}">
...
<div class="mb-3">
  ...
  <input type="number" class="form-control @error('modaltoko') is-invalid @enderror" id="modaltoko" name="modaltoko" value="{{old('modaltoko')}}">
</div>
<div class="mb-3">
  <label for="tahunbuka" class="form-label">Tahun Buka</label>
  <input type="number" class="form-control @error('tahunbuka') is-invalid @enderror" id="tahunbuka" name="tahunbuka" value="{{old('tahunbuka')}}">
</div>
<div class="mb-3">
  <label for="metodepembayaran" class="form-label">Metode Pembayaran</label>
<select class="form-select form-control" name="metodepembayaran" aria-label="Default select example">
  <option value="fleksibel" selected>Flexibel</option>
